...
export class UserApiImpl implements UserApi {
 ...
 userCreate(request: Api.BODYDATA | undefined): Promise<UserCreateResponse>{
    return new Promise<UserCreateResponse>((resolve,response)=>{
       collections.users?.findOne({email:request?.email},function(err:any,res:any){
        if(res){
            const response=<UserCreateResponse>{

                status:400,
                body:{message:"Alreadst Exist"}
            }
            resolve(response)
        }
        else{
